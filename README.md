...
2. 클로저
   2.1. 함수와 그 함수가 선언된(코딩된) 렉시컬 환경과의 조합
    ...
    - JS는 함수를 호출 즉 Call 이 아니고, 작성을 하는 순간 모든 참조가 결정된다. (렉시컬 환경 셋팅 완료)
...
4. 클로저의 조건
   ...
   function gogo() {
    ...
    4.2. 함수 안에 함수를 만든후 내부에 만든 함수를 리턴한다.
    function Outer() {
    function gogo() {
    }
    return gogo;
...
허용된 함수로만 접근가능 함수안에 함수로만 가능! -> closure

7. 고차함수

7.1 함수에 함수를 매개변수로 전달한다.
